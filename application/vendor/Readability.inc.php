...
class Readability {
    ...
    // 需要删除的属性
    private $junkAttrs = Array("style", "class", "onclick", "onmouseover", "align", "border", "margin","href");
...
public function getArticleImage($article) {
        ...
        if ($images->length) {
            ...
            while ($images->length) {
                ...
                array_push($imageArray, $source);
                $uuid = "imgOfLazyCat$index";
                // 创建新的标签
                $newTag = $article->createElement('nobr',$uuid);

                $img->parentNode->replaceChild($newTag,$img);
                $index++;
            }
        }
        return $imageArray;
    ...
    public function getContent() {
        ...
        // 获取图片并替换标签
        $imageArray = $this->getArticleImage($Target);
